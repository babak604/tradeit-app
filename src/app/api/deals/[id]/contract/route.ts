...
export async function GET(
  ...
  const contractHtml = `
    ...
    <body>
      ...
      <div class="grid">
        <div class="box">
          <div class="box-title">PARTY A: ${dealData.offer_a.company.name} (${dealData.offer_a.company.location_name})</div>
          <div style="font-size: 14px; font-weight: bold; margin-bottom: 6px;">${dealData.offer_a.title}</div>
          <div style="font-size: 12px; color: #475569; margin-bottom: 12px;">${dealData.offer_a.offering_summary}</div>
          <div class="value">Valuation: $${dealData.offer_a.estimated_value.toLocaleString()} USD</div>
        </div>

        <div class="box">
          <div class="box-title">PARTY B: ${dealData.offer_b.company.name} (${dealData.offer_b.company.location_name})</div>
          <div style="font-size: 14px; font-weight: bold; margin-bottom: 6px;">${dealData.offer_b.title}</div>
          <div style="font-size: 12px; color: #475569; margin-bottom: 12px;">${dealData.offer_b.offering_summary}</div>
          <div class="value">Valuation: $${dealData.offer_b.estimated_value.toLocaleString()} USD</div>
        </div>
      </div>
      ...
      <div class="signatures">
        ...
        <div class="sig-box">
          <strong>Party A Authorized Execution:</strong> ${dealData.offer_a.company.name}<br>
        ...
        <div class="sig-box" style="margin-top: 15px;">
          <strong>Party B Authorized Execution:</strong> ${dealData.offer_b.company.name}<br>
