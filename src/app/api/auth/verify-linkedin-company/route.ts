// src/app/api/auth/verify-linkedin-company/route.ts
...
export async function POST(req: NextRequest) {
  try {
    const { companyId, linkedinOrgUrn, accessToken } = await req.json();
    ...
    if (accessToken && linkedinOrgUrn) {
      try {
        ...
        if (linkedinRes.ok) {
          const aclData = await linkedinRes.json();
          // Verify user has admin rights to the requested organization URN
          const hasAdminAccess = aclData.elements?.some((element: any) =>
    ...
    return NextResponse.json(
      { error: err.message || 'Internal server error verifying corporate identity.' },
      { status: 500 }
    );
  }
}